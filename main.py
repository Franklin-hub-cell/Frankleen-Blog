    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            try:
                ...
                flash("Email already in use!, use the email to sign in instead.", category="error")
...
def login():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if not user:
                flash("Crendentials not found", "error")
                return redirect(url_for("login"))
...
def show_post(post_id):
    ...
    if request.method == 'POST':
        ...
        if not current_user.is_authenticated:
            flash("You would have to log in before you comment", "error")
            return redirect(url_for('login'))
        else:
